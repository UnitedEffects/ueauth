...
class NatsConnector {
	...
	async connect() {
		try {
			...
			this.nc = await connect({ name: 'stream-saver', servers: this.nats, authenticator: credsAuthenticator(new TextEncoder().encode(this.creds)), inboxPrefix: this.inbox, debug: this.debug });
