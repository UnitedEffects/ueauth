...
export default {
	updateSwag(swag, authGroup, params, cd = undefined) {
		...
		swag.info['x-logo'].url = (authGroup?.config?.ui?.skin?.logo) ?
			authGroup?.config?.ui?.skin?.logo :
			(authGroup?.config?.ui?.skin?.splashImage) ?
				authGroup?.config?.ui?.skin?.splashImage :
				pJson.logo;
		swag.info.description = (config.ENV === 'qa') ? `${swag.info.description}<h4 style='color:red'>WARNING: THIS IS A DEMO AND TEST ENVIRONMENT. ALL DATA IS EPHEMERAL AND SUBJECT TO DELETION.</h4>` : swag.info.description;
		...
		if (config.SWAGGER) swag.servers = [{url: `${config.PROTOCOL}://${cd || config.SWAGGER}`}];
		if(params.group) {
			swag.components.securitySchemes.openId.openIdConnectUrl = `${config.PROTOCOL}://${cd || config.SWAGGER}/${authGroup.id}/.well-known/openid-configuration`;
